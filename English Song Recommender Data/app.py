...
def recommend(song):
    ...
    for x in distances[1:6]:
        artist = music.iloc[x[0]].artist
        song_name = music.iloc[x[0]].song
...
music = pickle.load(open("english_df", "rb"))
similarity = pickle.load(open("english_similarity", "rb"))
...
st.markdown(f"<h1 style='color: {button_color}; text-align: center;'>English Song Recommender</h1>", unsafe_allow_html=True)
